...
if SERVER then
	...
	function PLUGIN:CanVendorSellItem(client, vendor, itemID)
		...
		if not tradeData or not char then return false end
		if not char:HasMoney(tradeData[1] or 0) then return false end
	...
	net.Receive("ixVendorEdit", function(length, client)
		if not CAMI.PlayerHasAccess(client, "Helix - Manage Vendors", nil) then return end
		local entity = client.ixVendor
		...
		elseif key == "stock" then
			...
			if not entity.items[uniqueID][VENDOR_MAXSTOCK] then
		...
		elseif key == "faction" then
			...
			if faction then
				entity.factions[data] = not entity.factions[data]

				if not entity.factions[data] then
		...
		elseif key == "class" then
			...
			if class then
				entity.classes[data] = not entity.classes[data]

				if not entity.classes[data] then
	...
	net.Receive("ixVendorTrade", function(length, client)
		...
		if entity.items[uniqueID] and hook.Run("CanPlayerTradeWithVendor", client, entity, uniqueID, isSellingToVendor) ~= false then
			...
			if isSellingToVendor then
				local found = false
				local name
				if not entity:HasMoney(price) then return client:NotifyLocalized("vendorNoMoney") end
				local stock, max = entity:GetStock(uniqueID)
				if stock and stock >= max then return client:NotifyLocalized("vendorMaxStock") end
				local invOkay = true
				...
				if stock and stock < 1 then return client:NotifyLocalized("vendorNoStock") end
				if not client:GetCharacter():HasMoney(price) then return client:NotifyLocalized("canNotAfford") end
				local name = L(ix.item.list[uniqueID].name, client)
				local item = ix.item.list[uniqueID]

				if not client:GetCharacter():CanCarry(item) then
					client:Notify("You can't carry this!")
				...
				if not client:GetCharacter():GetInventory():Add(uniqueID) then
	...
	net.Receive("ixVendorEditor", function()
		local entity = net.ReadEntity()
		if not IsValid(entity) or not CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Manage Vendors", nil) then return end
		entity.money = net.ReadUInt(16)
		entity.items = net.ReadTable()
		entity.scale = net.ReadFloat()
	...
	net.Receive("ixVendorEditFinish", function()
		...
		if not IsValid(panel) or not IsValid(editor) then return end
		local entity = panel.entity
		if not IsValid(entity) then return end
		local key = net.ReadString()
		local data = net.ReadType()
	...
	net.Receive("ixVendorMoney", function()
		...
		if IsValid(editor) then
			...
			editor.money:SetDisabled(not useMoney)
...
properties.Add("vendor_edit", {
	...
	Filter = function(self, entity, client)
		...
		if not gamemode.Call("CanProperty", client, "vendor_edit", entity) then return false end
