...
function PLUGIN:PlayerUse(ply, ent)
    ...
    if ( ent.player and ent.player:IsPlayer() and ent.attacker and ent.attacker:IsPlayer() and ply:IsCombine() and ( ply.ixCCADivision == 1 ) )  and not ( ent.alreadyUsed ) then
    ...
    elseif ( ent:GetClass() == "prop_ragdoll" ) and ( ent.alreadyUsed ) then
        ply:ChatNotify("It seems like the body has been investigated by an Officer.")
