...
function PANEL:Init()
    ...
    self.bottomBox:SetPaintBackground(false)
...
function PANEL:BuildRecordAdd(editMode)
    ...
    local finishButton = self.recordAdd:Add("DButton")
...
function PANEL:EditRecord(data)
    if not data.title or not data.index then return end
...
function PANEL:GetSelectedRow()
    if self.record:GetSelectedLine() then return self.record:GetSelectedLine() end
