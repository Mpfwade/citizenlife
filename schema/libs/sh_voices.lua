...
Schema.voices.AddClass("UniversalUnionAdmin", function(ply)
	return ply:IsCombine() and ply:IsAdmin()
end)
...
Schema.voices.AddClass("CitizenMale", function(ply)
	...
	if ( ( ply:IsCitizen() or ply:IsCWU() ) and not ply:IsFemale() ) then
		return true
	end
end)
...
Schema.voices.AddClass("CitizenFemale", function(ply)
	local factionTable = ix.faction.Get(ply:Team())
	if ( ( ply:IsCitizen() or ply:IsCWU() ) and ply:IsFemale() ) then
		return true
	end
end)