...
function PANEL:Init()
	...
	local xp = tonumber(LocalPlayer():GetLP())

	for k, v in pairs(ix.divisions.cwu) do
		if (xp >= v.xp) then
			self.jobsBox:AddChoice(ix.divisions.cwu[k].name.." - "..ix.divisions.cwu[k].xp.." LP", k, false, "icon16/tick.png")
		else
			self.jobsBox:AddChoice(ix.divisions.cwu[k].name.." - "..ix.divisions.cwu[k].xp.." LP", k, false, "icon16/cross.png")
		end
	end
...
vgui.Register("LiteNetwork.RankMenu.CWU", PANEL, "DFrame")