...
function PANEL:Init()
	self:SetIconSize(ScreenScale(30))
...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
	...
	tabs["character"] = {
		...
		Create = function(info, container)
			...
			playermodel:SetSize(container:GetWide() / 3, 0)
			playermodel:SetModel(LocalPlayer():GetModel() or "models/player/kleiner.mdl")
			playermodel:SetFOV(ScreenScale(14))

			function playermodel:LayoutEntity()
			...
			timer.Simple(0, function()
				if not IsValid(playermodel) then
					return
				end
	
				local ent = playermodel.Entity
	
				if not (ent:GetModel() == LocalPlayer():GetModel()) then
