...
function PANEL:SetRecipe(recipeTable)
	...
	self.name:SetFont("ixSubTitleFont")
...
function PANEL:Init()
	...
	for category, realName in SortedPairs(self.categoryPanels) do
		...
		button.name = button:Add("DLabel")
		button.name:Dock(FILL)
		button.name:DockMargin(10, 0, 0, 0)
		button.name:SetContentAlignment(4)
		button.name:SetTextColor(color_white)
		button.name:SetFont("ixSubTitleFont")
		button.name:SetExpensiveShadow(1, Color(0, 0, 0, 200))
		button.name:SetText(category)
	...
	--[[local craftButton = self.scroll:Add("ixMenuButton")
	craftButton:Dock(TOP)
	craftButton:SetText("")
	craftButton:SizeToContents()
	craftButton.DoClick = function(this)
	end

	craftButton.name = craftButton:Add("DLabel")
	craftButton.name:Dock(BOTTOM)
	craftButton.name:DockMargin(10, 0, 0, 0)
	craftButton.name:SetContentAlignment(5)
	craftButton.name:SetTextColor(color_white)
	craftButton.name:SetFont("ixSubTitleFont")
	craftButton.name:SetExpensiveShadow(1, Color(0, 0, 0, 200))
	craftButton.name:SetText("Craft Item")]]
end

function PANEL:LoadRecipes(category, search)
	category = category	or "Cooking"
