...
function PLUGIN:UpdateHunger(ply, char, hungerLevel)
    ...
    if factionIgnore[ply:Team()] or ply:GetMoveType() == MOVETYPE_NOCLIP then return end
...
function PLUGIN:IncreaseHungerForCCA()
    for _, ply in ipairs(player.GetAll()) do
        if ply:Team() == FACTION_CCA then
            local char = ply:GetCharacter()
            if char then
                local currentHunger = char:GetHunger()
                local newHunger = math.min(currentHunger + 25, 100)  -- Assuming 100 is max hunger
                char:SetHunger(newHunger)

                -- Play sounds
                ply:EmitSound("npc/overwatch/radiovoice/antifatigueration3mg.wav", 75, 100, 1, CHAN_AUTO)
                ply:EmitSound("player/suit_sprint.wav", 75, 100, 1, CHAN_AUTO)

                -- Notify the player
                ply:ChatNotify("You have received your anti-fatigue ration.")
            end
        end
    end
end

function PLUGIN:StartHungerIncreaseTimer(client)
    local timerName = "CCA_HungerIncrease_" .. client:SteamID()  -- Unique timer name for each client

    if not timer.Exists(timerName) then  -- Check if the timer already exists
        timer.Create(timerName, 600, 0, function()  -- Create a new timer if not exists
            if IsValid(client) and client:Team() == FACTION_CCA then
                self:IncreaseHungerForCCA(client)
            else
                timer.Remove(timerName)  -- Remove the timer if the condition no longer applies
            end
        end)
    end
end

-- Function to stop the hunger increase timer for a client
function PLUGIN:StopHungerIncreaseTimer(client)
    local timerName = "CCA_HungerIncrease_" .. client:SteamID()  -- Same unique timer name

    if timer.Exists(timerName) then
        timer.Remove(timerName)  -- Remove the timer if it exists
    end
end
...
function PLUGIN:OnPlayerAreaChanged(ply, newArea, oldArea)
    local char = ply:GetCharacter()
    if not char then
        print("Character not found for player:", ply)  -- Debug output
        return
    end

    local currentSanity = char:GetSanity()
    print("Area changed from", oldArea, "to", newArea, "with sanity", currentSanity)  -- Debug output

    if currentSanity < 14 then
        if newArea == "Residential Block 1" then
            net.Start("StopClientSound")
            net.Send(ply)
            net.Start("PlayClientSound")
            net.WriteString("citizensounds/insanity_outside.mp3")
            net.Send(ply)
        elseif oldArea == "Residential Block 1" then
            net.Start("StopClientSound")
            net.Send(ply)
            net.Start("PlayClientSound")
            net.WriteString("citizensounds/insanity_apartment.mp3")
            net.Send(ply)
        end
    else
        net.Start("StopClientSound")
...
function PLUGIN:UpdateStress(ply)
    ...
    local baseDecay = 1
    local stressIncreaseRate = 3
    local stressDecayRate = 0.5
    local updateInterval = 2

    if not ply.nextStressUpdate or ply.nextStressUpdate <= CurTime() then
        ply.nextStressUpdate = CurTime() + updateInterval
        local newStress = stressLevel
        
        -- Increase stress if a recent death was nearby and the player wasn't the attacker
        if ply:GetNWBool("recentDeathNearby") then
            newStress = math.min(100, newStress + 10)  -- Increase stress more significantly
        end

        
        local ccaNearby = ents.FindInSphere(ply:GetPos(), 500)
        local ccaCount = 0
        for _, ent in ipairs(ccaNearby) do
            if IsValid(ent) and ent:IsPlayer() and ent:Team() == FACTION_CCA then
                ccaCount = ccaCount + 1
            end
        end
        if ccaCount > 1 then
            newStress = math.min(100, newStress + 5 * ccaCount)  -- Increasing stress based on CCA presence
            print("nearby bad guys")
        end

        local projectilesNearby = ents.FindByClass("ls_projectile")
        for _, projectile in ipairs(projectilesNearby) do
            if ply:GetPos():Distance(projectile:GetPos()) < 200 then
                newStress = math.min(100, newStress + 15)  -- Large stress jump for proximity to danger
                print("projectiles nearby")
                break
            end
        end
    ...
    -- Decrement stress gradually over time
    if stressLevel > 0 then
        local newStress = math.max(0, stressLevel - stressDecayRate)
        char:SetData("stress", newStress)
    end

    -- Adjust sanity decay based on stress
    local stressFactor = stressLevel * stressImpactFactor
    local decayRate = baseDecay + stressFactor
    self:UpdateSanity(ply, decayRate)

    -- Reset recent death flag with a timer to allow stress increase to be triggered more realistically
    if ply:GetNWBool("recentDeathNearby") then
    print("nearby death")
        timer.Simple(30, function()  -- Resets the flag after 30 seconds
            if IsValid(ply) then
