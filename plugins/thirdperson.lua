...
if (CLIENT) then
    ...
    function PLUGIN:CalcView(ply, origin, angles, fov)
        ...
        if (ply and ply:Alive()) and not (ply:GetMoveType() == MOVETYPE_NOCLIP) then
            local newOrigin = ply:EyePos() - (view.angles:Forward() * ix.option.Get("thirdpersonDistance", false)) + (view.angles:Right() * ix.option.Get("thirdpersonHorizontal", false)) + (view.angles:Up() * ix.option.Get("thirdpersonVertical", false))
            ...
            if ix.option.Get("thirdpersonEnabled") then
                if not (IsValid(ix.gui.characterMenu) or (IsValid(ix.gui.menu) and IsValid(ix.gui.menu.projectedTexture))) then
                    if not (ply:GetNWEntity("ixRappeling") == true) then return view end
                end
            else
                headPosLerp = origin
                headAngleLerp = angles
