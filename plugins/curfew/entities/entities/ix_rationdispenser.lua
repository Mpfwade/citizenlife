...
if ( SERVER ) then
	...
	function ENT:Think()
		if (self.nextTerminalCheck or 0) < CurTime() then
			self:StopSound("ambient/machines/wall_loop1.wav")
			self:EmitSound("ambient/machines/wall_loop1.wav", 70)
						...
						timer.Simple(1.75, function()
							...
							entity:CallOnRemove("CreateRation", function()
								if (IsValid(activator)) then
									if char:GetInventory():FindEmptySlot(1, 1, true) then
										char:GetInventory():Add("ration", 1)
									elseif not char:GetInventory():FindEmptySlot(1, 1, true) then
