...
function PLUGIN:PlayerDeath(client, inflictor, attacker)
	...
	if (IsValid(camera)) then
		if (Schema.AddCombineDisplayMessage) then
			Schema:AddCombineDisplayMessage("@relationArea", Color(255, 0, 0), 8, client:Name())
		end

		if (Schema.AddWaypoint) then
			Schema:AddWaypoint(client:GetPos(), "@relationArea", Color(255, 0, 0), 30, nil, client:Name())
		end
	end
end
