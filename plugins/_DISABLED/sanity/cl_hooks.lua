...
PLUGIN.randomMessages = {
  "are the combine ever going away...?",
  "why does the world suck ass so much...?",
...
PLUGIN.events = {
  ...
  [ 8 ] = function( client )
    ...
    local sound = CreateSound( client, "gbombs_5/explosions/nuclear/nuke_trinity.mp3" )
    sound:SetDSP(38)
    sound:Play()
  ...
  for i, v in ipairs( self.monsters ) do
    ...
    if ( v.ent ) then
      ...
      if ( client:GetPos():DistToSqr( v.pos ) < self.dist ) then
        ...
        surface.PlaySound( "gbombs_5/explosions/light_bomb/small_explosion_5.mp3" )
