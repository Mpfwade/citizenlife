...
PLUGIN.chockSounds = {
    ...
    [3] = 'citizensounds/outofbreath.wav'
}
...
function PLUGIN:CreateGasTimer(client, character)
    if not IsValid(client) and not character then return end
    ...
    local cantInfect = {
        client:GetMoveType() == MOVETYPE_NOCLIP,
        client:IsCombine(),
        character:GetClass() == CLASS_CWU_ICT,
        client:GetModel() == "models/hazcit_npc.mdl"
    }

    timer.Create("GasTick." .. uniqueID, thinkDelay, 0, function()
        if not IsValid(client) or not character or not hook.Run("ShouldCharacterPoisoned", client, character) then
            return
        end
        
        if cantInfect[1] or cantInfect[2] or cantInfect[3] or cantInfect[4] then
            return
        end
...
function PLUGIN:OnVarChanged(client, character, value)
    ...
    if value <= 20 then
        if timer.Exists("GasKillCD." .. uniqueID) then
            timer.Remove("GasKillCD." .. uniqueID)
        end

        if timer.Exists("GasCoughTimer." .. uniqueID) then
            timer.Remove("GasCoughTimer." .. uniqueID)
        end
...
function PLUGIN:DeathTimerCountdown(client, character)
    ...
    timer.Create("GasKillCD." .. uniqueID, thinkDelay, 0, function()
        ...
        if not inGasArea and client:GetMoveType() ~= MOVETYPE_NOCLIP then
            if client:IsFemale() then
                client:EmitSound("ambient/voices/cough3.wav")
            else
                client:EmitSound(randomCough)
