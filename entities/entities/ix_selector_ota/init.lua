...
concommand.Add("ix_selector_ota", function(ply, cmd, args)
	...
	if not ( ( ply.ixOverwatchCoolDown or 0 ) > CurTime() ) then
		...
		local RandomNumbers = Schema:ZeroNumber(math.random(1, 9), 1)
		...
		local RankInfo = ix.ranks.ota[RankID]
		local CommandingName = "OTA:"..DivisionInfo.name.."-"..RandomNumbers
		local StandardName = "OTA:"..RankInfo.name.."."..DivisionInfo.name.."-"..RandomNumbers
		...
		if ( DivisionInfo.norank == true ) then
			if CommandingName:find("OWC") and not (ply:SteamID() == "STEAM_0:0:503591671" or ply:IsSuperAdmin()) then
				ply:Notify("The Commander Division is whitelisted!")
				return false
			end

			if (CommandingName:find("OWC") and not (ply:SteamID() == "STEAM_0:0:503591671")) and ply:IsSuperAdmin() then
			...
			if ( RankInfo.name == "ORDINAL" ) then
				ply:SetModel(DivisionInfo.model_ldr)
				ply:SetSkin(DivisionInfo.skin_ldr)
			else
				ply:SetModel(DivisionInfo.model)
				ply:SetSkin(DivisionInfo.skin)
			if ( RankInfo.name == "EOW" ) then
				ply:SetModel(DivisionInfo.model_eow)
				ply:SetSkin(DivisionInfo.skin_eow)
			else
				ply:SetModel(DivisionInfo.model)
				ply:SetSkin(DivisionInfo.skin)
			end
