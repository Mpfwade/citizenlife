...
if CLIENT then
    netstream.Hook("OpenCIDMenu", function(data) vgui.Create("ixCIDCreater") end)
    netstream.Hook("OpenCCAMenu", function(data) vgui.Create("ixCCACreater") end)
    netstream.Hook("OpenQuizMenu", function(data) vgui.Create("ixJobQuiz") end)
...
if SERVER then
    netstream.Hook("SubmitNewCID", function(client, data)
        if client:IsCombine() or client:GetCharacter():HasFlags("i") then
            ...
            if data.item then inventory:Remove(data.item) end
    ...
    netstream.Hook("SubmitNewCCA", function(client, data)
        if client:IsCombine() or client:GetCharacter():HasFlags("i") then
            local character = client:GetCharacter()
            local inventory = character:GetInventory()
            if not character or not inventory then return end
    ...
    netstream.Hook("SubmitCPPaper", function(client, applicationText)
        if client:Team() == FACTION_CITIZEN then
            ...
            if character then
                ...
                print("Application text:", applicationText) -- Store the application text in a database or perform any other necessary operations
            end
        end
    end)
    ...
    ix.command.Add("Apply", {
        ...
        OnRun = function(_, ply)
            ...
            elseif (ply:Team() == FACTION_CITIZEN) and char:GetInventory():HasItem("cca_id") then
                ply:EmitSound("physics/cardboard/cardboard_box_impact_soft7.wav")
                ix.chat.Send(ply, "me", "shows their identification card showing: (Name: " .. ply:Nick() .. " | CID: " .. char:GetData("cid", "00000") .. " the card has a small civil protection badge imprinted onto it" .. ") ", false)
                ply:ConCommand("ix_act_Point")
